...
EOD;
	...
	public function config_handler() {
  	...
  	//
		if ( isset($_POST["hide-wp-config-submit"]) && $_POST["hide-wp-config-submit"] == 'Y' ) {
			//echo 1;
    ...
    foreach( $tabs as $tab => $name ){
        ...
        echo "<a class='nav-tab$class' href='?page=hide_wp&tab=$tab' title='".$tabs_hints[$tab]."'>$name</a>";
    }
    echo '</h2>';		
    $form_method = 'POST';
    if (1==2 && $tab == 'log') {
    ...
    if ($current == 'other') {
        ...
        <tr>
        	<th></th>
            <td>
               <input id="w3tc_can_print_comment" name="w3tc_can_print_comment" type="checkbox" value="1" <?php checked((isset($settings['w3tc_can_print_comment']) && $settings['w3tc_can_print_comment'] == '1')?'1':'0','1',true); ?>/> <label for="w3tc_can_print_comment">Disable W3 Total Cache (if installed) comment in page footer.</label><br/>
               <span class="description"></span>
            </td>
        </tr>
    		</table>
    ...
    if ($current == 'plugins') {
        ...
        <tr>
	        ...
	        <tr>
  	      	<th></th>
    	        <td>
      	         <input id="block_<?php echo $slug; ?>" name="plugins_block[<?php echo $slug; ?>]" type="checkbox" value="1" <?php checked((isset($settings['plugins_block'][$slug]) && $settings['plugins_block'][$slug] == 1)?'1':'0','1',true); ?>/> <label for="block_<?php echo $slug; ?>">Block oryginal plugin URL</label><br/>
        	       <span class="description"></span>
          	  </td>
        	</tr>
					<?php
